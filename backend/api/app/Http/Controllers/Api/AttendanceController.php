...
class AttendanceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $records = $q->orderByDesc('date')->limit(200)->get();

        // For single-user monthly view, represent absent as missing DB rows.
        if (!is_null($year) && !is_null($month) && !is_null($targetUserId)) {
            ...
            $recordsByDate = $records->keyBy(fn ($r) => Carbon::parse($r->date)->toDateString());
            $result = collect();
            ...
            while ($cursor->lte($end)) {
                ...
                if ($cursor->isWeekend() || $holidayDates->has($dateKey)) {
                    $cursor->addDay();
                    continue;
                }

                if ($recordsByDate->has($dateKey)) {
                    $result->push($recordsByDate->get($dateKey));
                } else {
                    $result->push([
                        'id' => 'absent-' . $targetUserId . '-' . $dateKey,
            ...
            return response()->json($result->sortByDesc(function ($item) {
                return is_array($item) ? $item['date'] : $item->date;
            })->values());
        }
    ...
    private function computeLocationStatus(string $workMode, float $lat, float $lng): array
    {
        ...
        $officeLat = (float) ($setting?->office_latitude ?? env('OFFICE_LATITUDE', -7.9397675));
        $officeLng = (float) ($setting?->office_longitude ?? env('OFFICE_LONGITUDE', 112.69277025));
